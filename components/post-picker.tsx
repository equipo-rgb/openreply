...
export default function PostPicker({
    ...
    fetch(`/api/instagram/posts${params.size ? `?${params}` : ""}`)
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          setError(data.error ?? "No se han podido cargar las publicaciones");
        }
      })
      .catch(() => {
        if (!cancelled && !cached.data) setError("No se han podido cargar las publicaciones");
      })
      .finally(() => {
  ...
  if (posts.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted">No se han encontrado publicaciones</p>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      ...
      {visible.length === 0 ? (
        <p className="py-6 text-center text-sm text-muted">
          Ninguna publicación coincide con &ldquo;{query}&rdquo;
        </p>
      ) : (
        <>
          ...
          >
            {thumb ? (
              <img
                src={thumb}
                alt={post.caption?.slice(0, 50) ?? "Publicación de Instagram"}
                loading="lazy"
                decoding="async"
