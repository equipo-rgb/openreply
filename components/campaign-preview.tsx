  ...
  return withName.split(/(\{link\})/g).map((part, i) =>
    part === "{link}" ? (
      ...
      >
        ...
        {linkUrl || (hasLink ? "tu enlace" : "{link}")}
      </span>
    ) : (
      ...
      <div className="shrink-0 px-3 text-xs leading-relaxed">
        <p className="line-clamp-2">
          ...
          <span className="text-zinc-200">
            {caption || "¡El plazo de inscripción termina muy pronto!"}
          </span>
        </p>
        <p className="mt-1 text-zinc-500">Ver todos los comentarios</p>
      </div>
  ...
  return (
    <div className="flex h-full flex-col text-white">
      ...
      <div className="flex flex-1 flex-col rounded-t-2xl bg-[#0b0b0b] px-4 pt-3">
        ...
        <p className="text-center text-sm font-semibold">Comentarios</p>
        ...
        {publicReplyEnabled && (
          <div className="mt-4 flex gap-3 pl-10">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm">{publicReplyMessage || "¡Te he enviado un DM!"}</p>
              <p className="mt-0.5 text-xs text-zinc-500">Responder</p>
            </div>
            <span className="mt-1">{Ico.heart("h-3.5 w-3.5 text-zinc-500")}</span>
          </div>
        ...
        <div className="mt-auto">
          ...
          <div className="mb-3 flex items-center gap-2">
            ...
            <div className="flex-1 rounded-full bg-zinc-800 px-3 py-2 text-xs text-zinc-500">
              Añade un comentario para {username}…
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex h-full flex-col text-white">
          ...
          <>
            <div className="flex items-end gap-2">
              ...
              <div className="max-w-[80%] overflow-hidden rounded-2xl rounded-bl-md bg-zinc-800">
                <p className="whitespace-pre-wrap px-3 py-2 text-sm">{openingDmMessage || "Tu mensaje inicial…"}</p>
                <div className="mx-1.5 mb-1.5 rounded-xl bg-zinc-700 px-4 py-1.5 text-center text-sm font-medium text-white">
                  {openingDmButtonLabel || "Texto del botón"}
                </div>
              </div>
            </div>
            <div className="flex justify-end">
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="max-w-[80%] overflow-hidden rounded-2xl rounded-bl-md bg-zinc-800">
                <p className="whitespace-pre-wrap px-3 py-2 text-sm">
                  {followPromptMessage ||
                    "Un favor antes de enviarte el enlace. Esto es gratis y no gano dinero con ello. Si quieres apoyarme, sigue la cuenta y dale una estrella al repositorio en GitHub si te resulta útil. Pulsa el botón cuando sigas la cuenta y te enviaré el enlace."}
                </p>
                <div className="mx-1.5 mb-1.5 rounded-xl bg-zinc-700 px-4 py-1.5 text-center text-sm font-medium text-white">
                  {followPromptButtonLabel || "Confirmar que sigo"}
                </div>
              </div>
            </div>
            <div className="flex justify-end">
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="max-w-[80%] overflow-hidden rounded-2xl rounded-bl-md bg-zinc-800">
                ...
                {showCard && (
                  <>
                    <div className="mx-1.5 mb-1.5 rounded-xl bg-zinc-700 px-4 py-1.5 text-center text-sm font-medium text-white">
                      {linkButtonLabel || "Abrir enlace"}
                    </div>
                    {hasSecondLink && (
                      <div className="mx-1.5 mb-1.5 rounded-xl bg-zinc-700 px-4 py-1.5 text-center text-sm font-medium text-white">
                        {secondLinkButtonLabel || "Abrir enlace"}
                      </div>
                    )}
                  </>
                )}
          ...
          <>
            {followUpDelayMinutes > 0 && (
              <p className="py-1 text-center text-[11px] text-zinc-500">
                {followUpDelayMinutes} min después
              </p>
            )}
            <div className="flex items-end gap-2">
              <Avatar url={avatarUrl} size={24} />
              <div className="max-w-[80%] rounded-2xl rounded-bl-md bg-zinc-800 px-3 py-2">
                <p className="whitespace-pre-wrap text-sm">
                  {followUpMessage.trim()
                    ...
                    : "Gracias por seguirme. Te agradezco el apoyo."}
                </p>
              </div>
            </div>
          </>
      ...
      <div className="flex items-center gap-2 px-3 py-3">
        ...
        <div className="flex-1 rounded-full bg-zinc-800 px-3 py-2 text-xs text-zinc-500">Mensaje…</div>
      </div>
    </div>
  );
}
...
export default function CampaignPreview(props: CampaignPreviewProps) {
  ...
  const tabs: { key: PreviewTab; label: string }[] = [
    ...
    ...(props.dmTriggerEnabled
      ? [{ key: "dmTrigger" as const, label: "Disparador por DM" }]
      : []),
  ];
